add_item = lambda current_cart, items_to_add: {i:([0,current_cart.get(i)][not not current_cart.get(i)] + items_to_add.count(i)) for i in {*items_to_add}|{*current_cart}}
...
send_to_store = lambda cart, aisle_mapping: sorted({item:[cart[item]]+aisle_mapping[item] for item in cart}.items(), key= lambda x: x[0], reverse=True)
